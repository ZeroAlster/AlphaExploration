...
import argparse
from DDPG_ICM.ddpg_icm import Agent
    ...
    while frame<warm_up:
        ...
        while not done:
            ...
            for action in option:
                ...
                # env density update
                agent.density_estimator.increment(state,agent.increment)
...
def main(address,environment,beta,increment):
    ...
    # initiate the agent
    agent=Agent(num_actions,num_states,action_range,density_estimator,threshold,beta,increment)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i','--increment',required=True)
    ...
    if args.task=="train":
        main(args.address,args.environment,float(args.beta),float(args.increment))
    elif args.task=="plot":
