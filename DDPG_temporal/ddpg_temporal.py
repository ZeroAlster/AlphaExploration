...
class Agent():
    ...
    def get_action(self, state,evaluation=False):
        ...
        if random.uniform(0, 1)<self.epsilon and not evaluation:
            # we will output an option by a unifrom distribution and a random action
            length=random.randrange(start=1,stop=self.option_length+1,step=1)
