...
def evaluation(agent,environment,env_test):
    # if environment=="maze":
...
def train(agent,env,address,environment,k,her_goal,test_env):
    ...
    while frame<max_frames:
        ...
        while not done:
            ...
            for action in option:
                ...
                frame+=1
                if frame % checkpoints_interval==0:
                    success_rates.append(evaluation(agent,environment,test_env))
                    env_coverages.append(exploration(env_density))
...
def main(address,environment,k):
    ...
    # train the agent
    train(agent,env,address,environment,k,HER_goal_reward,test_env)
